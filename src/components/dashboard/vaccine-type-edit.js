...
class VaccineTypeEdit extends React.Component {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    let id = Number(window.location.pathname.replaceAll('/vaccine-type-edit/', ''));
    this.setState({vaccineTypeID: id});
    let response = await axios.get(`${Config.API_URL_2}/admin_vaccine/detail_location_vaccine_type/${id}`, { headers: { 'token': this.state.admin.token } });
    try {
      ...
      if(data.statusCode === 200) {
        ...
        this.setState(prevState => ({
          form: {
            ...prevState.form,
            locationTypeVal: item.type_location,
            vaccineType: item.vaccine_name,
            minAge: item.min_age,
            maxAge: item.max_age,
            category: String(item.is_category),
          }
        }));
  ...
  async submit() {
    ...
    let response = await axios.post(`${Config.API_URL_2}/admin_vaccine/edit_location_vaccine_type/${this.state.vaccineTypeID}`, formData, { headers: { 'token': this.state.admin.token } });
    try {
      let data = response.data;
      if(data.statusCode === 200) {
          ...
          <div className="title d-flex flex-column flex-md-row">
            <div className="flex-grow-1 align-self-center">
              <div className="left d-flex">
                ...
                <div className="text flex-grow-1 align-self-center">Ubah Tipe Vaksin</div>
              </div>
            </div>
          </div>
        </div>
...
export default VaccineTypeEdit;
