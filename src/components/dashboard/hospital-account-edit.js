...
class HospitalAccountEdit extends React.Component {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    ...
    let id = Number(window.location.pathname.replaceAll('/hospital-account-edit/', ''));
    this.setState({hospitalID: id});
    let response = await axios.get(`${Config.API_URL_2}/admin_vaccine/data_account_hospital_details/${id}`, { headers: { 'token': this.state.admin.token } });
    try {
      let data = response.data;
      if(data.statusCode === 200) {
        let item = data.date[0];
        let descriptionBefore = item.description.length;
        do {
          descriptionBefore = item.description.length;
          item.description = item.description.replaceAll('  <', '<');
          item.description = item.description.replaceAll('\n<', '<');
          item.description = item.description.replaceAll('>  ', '>');
          item.description = item.description.replaceAll('>\n', '>');
        } while(item.description.length != descriptionBefore);
        console.log(item);
        this.setState(prevState => ({
          form: {
            ...
            name: item.name,
            email: item.email,
            category: Number(item.status) === 1 ? '1' : '0',
            editor: editor,
          },
        }), () => this.state.form.editor.setEditorValue(item.description));
  ...
  async submit() {
    ...
    if(this.state.form.password != '' && !this.state.form.password.match(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z]{8,}$/)) return Swal.fire('Kata sandi wajib mengandung angka, huruf kecil, dan huruf besar');
    ...
    let response = await axios.post(`${Config.API_URL_2}/admin_vaccine/update_account_hospital/${this.state.hospitalID}`, formData, { headers: { 'token': this.state.admin.token } });
    try {
      let data = response.data;
      if(data.statusCode === 200) {
          ...
          <div className="title d-flex flex-column flex-md-row">
            <div className="flex-grow-1 align-self-center">
              <div className="left d-flex">
                ...
                <div className="text flex-grow-1 align-self-center">Ubah Data Rumah Sakit</div>
              </div>
            </div>
          </div>
        </div>
        <div className="standard-form">
          <div className="d-flex flex-column flex-md-row">
            <div className="left flex-grow-1 flex-basis-0">
              ...
              <div className="group">
                <label>Kata Sandi <small><em>*kosongkan jika tidak berubah</em></small></label>
              ...
              <div className="group">
                <label>Konfirmasi Kata Sandi <small><em>*kosongkan jika tidak berubah</em></small></label>
                <div className="password-wrapper">
...
export default HospitalAccountEdit;
