...
class VaccineSlotCreate extends React.Component {
  constructor(props) {
    ...
    this.toggleBrandStatus = this.toggleBrandStatus.bind(this);
    this.toggleBrandStatusSecond = this.toggleBrandStatusSecond.bind(this);
  ...
  getLocationVaccineID = async () => {
    if (_.isEmpty(this.state.form.hospitalList)) {
    } else {
      await this.setState((prevState) => ({
        form: {
          ...prevState.form,
          location_vaccine_id:
            this.state.form.hospitalList[Number(this.state.form.hospitalVal)]
              .location_vaccine_id,
        },
      }));
  ...
  getBrandVaccineAllowed = async () => {
    if (
      this.state.form.location_vaccine_id == "" ||
      this.state.form.type_vaccine == ""
    ) {
      return;
    } else {
      let formData = {
        location_vaccine_id: this.state.form.location_vaccine_id,
        type_vaccine: this.state.form.type_vaccine,
      };
  ...
  async submit() {
    ...
    let response = await axios.post(
      `${Config.API_URL_2}/v2/admin_vaccine/add_slot_and_type_vaccine`,
      formData,
      { headers: { token: this.state.admin.token } }
    );
    try {
  ...
  async addFirstloc() {
    ...
    if (this.state.form.firstlocName === "")
      return Swal.fire("Nama Lokasi Sebelumnya wajib diisi");
    this.setState({ isLoading: true });
    let formData = {};
    formData.location_vaccine_id =
      this.state.form.hospitalList[
    ...
    try {
      ...
      if (data.statusCode === 201) {
        await Swal.fire(
          "Success",
          "Lokasi Vaksin Sebelumnya Berhasil ditambahkan",
          "success"
        );
  ...
  async editFirstloc() {
    ...
    try {
      ...
      if (data.statusCode === 200) {
        await Swal.fire(
          "Success",
          "Status lokasi vaksin sebelumnya berhasil diubah",
          "success"
        );
  ...
  async deleteFirstloc(firstloc) {
    if (this.state.isLoading) return;
    let confirm = await Swal.fire({
      title: "Hapus Lokasi Vaksin Sebelumnya ini?",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
    ...
    try {
      ...
      if (data.statusCode === 200) {
        await Swal.fire(
          "Success",
          "Lokasi Vaksin Sebelumnya berhasil dihapus",
          "success"
        );
        this.fetchFirstlocList();
      } else {
                ...
                <div className="date-slot">
                  <div className="container">
                    ...
                    <div className="apply-button">
                      <div onClick={this.applyRegularSlot}>Terapkan Semua</div>
                    </div>
                  </div>
