...
const EditInformationForm = (props) => {
  ...
  useEffect(() => {
    ...
    axios.post(Config.API_URL + "/get-information", formData, { headers: { 'token': admin.token } }).then(function(response) {
      try {
        let data = response.data;
        if(data.status) {
          let information = data.data;
          form.values.title = information.title;
          form.values.hospital = information.hospital_id;
          setContent(information.description);
        } else {
          Swal.fire('Failed', data.message, 'error');
        }
      } catch(error) {
  ...
  const form = useFormik({
    ...
    onSubmit: async values => {
      ...
      if(validator.isEmpty(String(values.hospital))) return Swal.fire('Failed', 'Pilih cabang rumah sakit', 'warning');
      ...
      if(coverImg !== undefined) formData.append('coverImg', coverImg);
      formData.append('content', content);
      let response = await axios.post(Config.API_URL + "/edit-information", formData, { headers: { 'token': admin.token } });
      try {
        let data = response.data;
        if(data.status) {
          await Swal.fire('Success', 'Informasi berhasil diubah', 'success');
        ...
        <div className="form-field mt-3">
          <div className="d-flex flex-column flex-md-row">
            <button onClick={triggerInputFile} type="button" className="transfer-proof">
              ...
              <span>{`Ubah Cover Informasi`}</span>
            </button>
