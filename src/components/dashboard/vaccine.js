...
class Vaccine extends React.Component {
  ...
  async getPatientList(page) {
    ...
    if (this.state.admin.authority === "Super Admin") {
      ...
      if (this.state.admin.hospitalName !== null)
        formData.hospital_name = this.state.admin.hospitalName;
    }
  ...
  async setPatient(patient) {
    ...
    try {
      ...
      if (data.statusCode === 200) {
        ...
        console.log(this.state.patient.val);
      } else {
        Swal.fire("Failed", data.message, "error");
      }
    } catch (error) {
  ...
  async cancelPatient() {
    ...
    try {
      let response = await axios.post(
        `${Config.API_URL_2}/delete_schedule_vaccine/${this.state.patient.val.schedule_vaccine_id}`
      );
      let data = response.data;
      if (data.statusCode === 200) {
  ...
  async fetchLocationVaccineCalendar() {
    ...
    formData.location_vaccine_id =
      this.state.patient.val.detail.location_vaccine_id;
  ...
  async exportExcel() {
    ...
    if (this.state.admin.authority === "Super Admin") {
      ...
      if (this.state.admin.hospitalName !== null)
        filter.hospital_name = this.state.admin.hospitalName;
    }
    if (this.state.form.filterVaccineType !== "")
      filter.type_vaccine =
        this.state.filter.filterVaccineType[
    ...
    if (this.state.form.filterBrand !== "")
      filter.brand = this.state.form.filterBrand;
    if (this.state.form.filterStatus !== "")
    ...
    if (this.state.form.filterVaccineDateFrom !== null)
      filter.date_schedule_from = moment(
        this.state.form.filterVaccineDateFrom
      ).format("YYYY-MM-DD");
    if (this.state.form.filterVaccineDateTo !== null)
      filter.date_schedule_to = moment(
        this.state.form.filterVaccineDateTo
      ).format("YYYY-MM-DD");
    let response = await axios.post(
      `${Config.API_URL_2}/download_data_vaccine`,
      filter,
      {
        headers: {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute("download", "Vaccine Data.xlsx"); //or any other extension
  ...
  getScreeningPatient(detail) {
    ...
    console.log("STATUS VAKSIN: ", detail.status_vaccine);
    switch (patientType) {
      case "ELDERLY":
        ...
        {this.state.patient.val == null && (
          <div className="patient-data-page standard-page">
            ...
            <div className="filter d-flex flex-wrap">
              {this.state.admin.authority === "Super Admin" && (
                ...
                <tbody>
                  ...
                  {this.state.patient.list.map((item, index) => (
                    <tr key={index}>
                          ...
                          <div
                            onClick={() =>
                              this.openUpdateStatusModal(
                                this.state.patient.list[index]
                              )
                            }
                            className="detail-text align-self-center clickable"
                            data-toggle="modal"
                            data-target="#status-vaccine-update-modal"
                          >
                            Ubah
                          </div>
                        </div>
                      ...
                      <td>
                        <div className="no-wrap">{item.email}</div>
                      </td>
                      <td>
        ...
        {this.state.patient.val != null && (
          <div className="patient-detail-page">
            <div className="top">
              ...
              {this.state.patient.val.detail != null && this.state.patient.val.detail.status === 'CANCELED' && <button className="canceled-button" disabled={true}>Telah Dibatalkan</button>}
              {this.state.patient.val.detail != null && this.state.patient.val.detail.status !== 'CANCELED' && <button onClick={this.cancelPatient} className="cancel-button">Batalkan</button>}
            </div> */}
              </div>
            </div>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="w-100 value">
                          ...
                          <i
                            onClick={() =>
                              this.openUpdateStatusModal(this.state.patient.val)
                            }
                            className="fa fa-edit clickable ml-2"
                            data-toggle="modal"
                            data-target="#status-vaccine-update-modal"
                          ></i>
                        </td>
