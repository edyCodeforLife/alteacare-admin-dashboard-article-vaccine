...
class SubscriberListCreate extends React.Component {
  constructor(props) {
    super(props);
  ...
  async submit() {
    ...
    try {
      ...
      formData.topic_id = this.state.topicSelected.map((item) => item.id);
      let response = await axios.post(`${Config.API_URL_2}/admin/subcribe`, formData, { headers: { token: this.state.admin.token } });
      let data = response.data;
      if(data.statusCode === 201 || data.statusCode === 200) {
        await Swal.fire('Sukses', 'Langganan sudah didaftarkan', 'success');
      ...
      let message = "Terjadi kesalahan pada koneksi anda. Silahkan coba beberapa saat lagi dan pastikan koneksi internet bekerja dengan baik.";
          ...
          <div className="title d-flex flex-column flex-md-row">
            <div className="flex-grow-1 align-self-center">
              <div className="left d-flex">
                ...
                <div className="text flex-grow-1 align-self-center">Tambah Langganan</div>
              </div>
            </div>
          </div>
        </div>
...
export default SubscriberListCreate;
