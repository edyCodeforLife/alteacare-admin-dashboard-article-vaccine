...
class HospitalVaccineCreate extends React.Component {
  constructor(props) {
    super(props);
  ...
  async submit() {
    ...
    if(!this.state.form.vaccine1 && !this.state.form.vaccine2) return Swal.fire('Wajib pilih Vaksinasi ke');
    let brand = '';
    ...
    this.setState({ isLoading: true });
    let formData = { };
    formData.location_vaccine_available = [];
    if(this.state.form.vaccine1) {
      formData.location_vaccine_available.push({
        location_vaccine_id: this.state.form.hospitalList[this.state.form.hospitalVal].location_vaccine_id,
        location_vaccine_type_id: this.state.form.vaccineTypeList[this.state.form.vaccineTypeVal].location_vaccine_type_id,
        vaccine_status: "PERTAMA",
        noted: this.state.form.locationTypeVal === 'UMUM' ? this.state.form.note : '',
        hospital_vaccine_notes: this.state.form.locationTypeVal === 'PERUSAHAAN' ? this.state.form.note : '',
        status: 1,
        brand_vaccine: brand,
        citizenship: citizenship,
      });
    }
    if(this.state.form.vaccine2) {
      formData.location_vaccine_available.push({
        location_vaccine_id: this.state.form.hospitalList[this.state.form.hospitalVal].location_vaccine_id,
        location_vaccine_type_id: this.state.form.vaccineTypeList[this.state.form.vaccineTypeVal].location_vaccine_type_id,
        vaccine_status: "KEDUA",
        noted: this.state.form.locationTypeVal === 'UMUM' ? this.state.form.note : '',
        hospital_vaccine_notes: this.state.form.locationTypeVal === 'PERUSAHAAN' ? this.state.form.note : '',
        status: 1,
        brand_vaccine: brand,
    ...
    formData.company = {
      location_vaccine_company: this.state.form.locationTypeVal === 'PERUSAHAAN' ? this.state.form.hospitalList[this.state.form.hospitalVal].name : '',
      passcode: this.state.form.locationTypeVal === 'PERUSAHAAN' ? this.state.form.passcode : '',
      company_name: this.state.form.locationTypeVal === 'PERUSAHAAN' ? this.state.form.companyName : '',
      type_location: this.state.form.locationTypeVal === 'PERUSAHAAN' ? this.state.form.locationTypeVal : '',
      type_vaccine: this.state.form.vaccineTypeVal !== '' ? this.state.form.vaccineTypeList[this.state.form.vaccineTypeVal].vaccine_name : '',
      birthdate: '2021-01-01',
      brand_vaccine: brand,
      citizenship: citizenship,
    };
    let response = await axios.post(`${Config.API_URL_2}/admin_vaccine/create_type_vaccine`, formData, { headers: { 'token': this.state.admin.token } });
    try {
      let data = response.data;
      if(data.statusCode === 200) {
          ...
          <div className="title d-flex flex-column flex-md-row">
            <div className="flex-grow-1 align-self-center">
              <div className="left d-flex">
                ...
                <div className="text flex-grow-1 align-self-center">Tambah Vaksin Rumah Sakit</div>
              </div>
            </div>
          </div>
        </div>
...
export default HospitalVaccineCreate;
